...
func addProfileFunc(cmd *cobra.Command, args []string) error {
	...
	if dupCount > 1 {
		...
		cmd.Println("Use `cloink profile list --show-id` to disambiguate later.")
	}
...
func wrapAmbiguityError(err error, handle string) error {
	...
	case codes.InvalidArgument:
		...
		if strings.Contains(msg, "ambiguous") {
			return errors.New(msg + "\nRun `cloink profile list --show-id` to see IDs, then select by ID prefix:\n  cloink profile select|remove <id-prefix>")
		}
	case codes.NotFound:
...
// creation, shared by `cloink profile add` and the `cloink up --profile
