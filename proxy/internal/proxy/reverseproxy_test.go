...
func TestClassifyProxyError(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantTitle:  "proxyError.requestTimeout",
		...
		},
		{
			...
			wantTitle:  "proxyError.requestCanceled",
		...
		},
		{
			...
			wantTitle:  "proxyError.configurationError",
		...
		},
		{
			...
			wantTitle:  "proxyError.proxyNotConnected",
			wantCode:   http.StatusBadGateway,
			wantStatus: web.ErrorStatus{Proxy: false, Destination: false},
		},
		{
		...
		},
		{
			...
			wantTitle:  "proxyError.serviceUnavailable",
			wantCode:   http.StatusBadGateway,
			wantStatus: web.ErrorStatus{Proxy: true, Destination: false},
		},
		{
		...
		},
		{
			...
			wantTitle:  "proxyError.peerNotConnected",
			wantCode:   http.StatusBadGateway,
			wantStatus: web.ErrorStatus{Proxy: true, Destination: false},
		},
		{
		...
		},
		{
			...
			wantTitle:  "proxyError.connectionError",
			wantCode:   http.StatusBadGateway,
			wantStatus: web.ErrorStatus{Proxy: true, Destination: false},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			title, message, code, status := classifyProxyError(tt.err)
			assert.Equal(t, tt.wantTitle, title, "title")
			assert.Equal(t, tt.wantTitle+"Message", message, "localized error message")
